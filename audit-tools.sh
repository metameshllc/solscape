...
dialogCheck() {
    if command -v dialog --help>/dev/null; then
        printf | dialog --clear --gauge "Dependency dialog passed." 10 70 3
    else
...
suryaCheck() {
    if command -v surya --help>/dev/null; then
        printf | dialog --clear --gauge "Dependency surya passed." 10 70 6
    else
...
graphvizCheck() {
    if command -v dot --help>/dev/null; then
        printf | dialog --clear --gauge "Dependency graphviz passed." 10 70 9
    else
...
getDir() {
    rootDir=$(dialog --title "Root Contract Directory" --stdout --title "Select the root contract directory and press spacebar then enter." --dselect ~/ 10 70)       
        printf | dialog --clear --gauge "Root directory acquired." 10 70 12
}

createReport() {
printf "# Scoping Report \n" > ScopingReport.md   
        printf | dialog --clear --gauge "Scoping report file created." 10 70 15
...
fileCount() {
  ...
  printf | dialog --clear --gauge "File count summed and written to ScopingReport.md" 10 70 18
...
lineCount() {
    ...
    printf | dialog --clear --gauge "Line count summed and written to ScopingReport.md" 10 70 21
}
...
suryaMdReport() {
    ...
    printf "* The description table of all contracts (`surya describe`.) \n" >> ScopingReport.md
      surya mdreport mdreport.md $suryaFilter > tmp.txt
    printf | dialog --clear --gauge "Markdown Report created." 10 70 24
      cat mdreport.md >> ScopingReport.md && printf "\n" >> ScopingReport.md
    printf | dialog --clear --gauge "Markdown Report written." 10 70 27
      rm tmp.txt mdreport.md
    printf | dialog --clear --gauge "Temporary files deleted." 10 70 30
}

suryaGraph() {
  printf "## Call Graph\n" >> ScopingReport.md
  printf "**Surya's Call Graph** creates an exhaustive visualization of all function calls." >> ScopingReport.md
  surya graph $suryaFilter  | dot -T png CallGraph.png
...
suryaGraph() {
  ...
  surya inheritance $suryaFilter  | dot -T png InheritanceGraph.png
  printf "![Inheritance Graph](InheritanceGraph.png)"
}

createAppendix() {
  printf "# Appendices\n" >> ScopingReport.md
  printf | dialog --clear --gauge "Appendix written." 10 70 33
}
...
suryaDescribe() {
    ...
    printf | dialog --clear --gauge "Description Report Written" 10 70 36
}

suryaParse(){
    printf "### Surya Parse\n" >> ScopingReport.md
    surya parse $suryaFilter >> ScopingReport.md
    printf | dialog --clear --gauge "Parse Report Written" 10 70 39
}
